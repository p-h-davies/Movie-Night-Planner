document.addEventListener('DOMContentLoaded', function () {
  ...
  function createRecipeCard(recipe) {
    ...
    saveButton.addEventListener('click', function () {
      saveDinner(recipe, saveButton); // Pass the save button as an argument
    });
  ...
  function saveDinner(recipe, button) {
    ...
    if (!savedDinners.some(savedRecipe => savedRecipe.id === recipe.id)) {
      ...
      displayMessage('Dinner saved successfully!', button); // Updated to pass the save button
      updateSavedDinners();
    } else {
      displayMessage('Dinner already saved!', button); 
    }
  }
  ...
  function displayMessage(message, result) {
    ...
    const resultContainer = result.closest('.card');
